...
do
	...
	do
		...
		while [ ${TSTRND} != 0 ]
		do
			echo "- -=[ Tour ${TOUR} / Round ${ROUND} ]=- -" >> ${FICLOG}
			echo " Ordre de Passage" >> ${FICLOG}
			for IDX in `seq 0 ${TMP}`
			do
				PLC=0	
			...
			do
				...
				then
					...
					then
						...
						then
							...
							then
								...
								mv ${CHNTAB[${INITAB[${IDXDEF} 'PNJ']} 'FIC']} ${ARNRIP}/
							fi
						else
					...
					CHNTAB[${INITAB[${IDXATT} 'PNJ']} 'Init']=`expr ${CHNTAB[${INITAB[${IDXATT} 'PNJ']} 'Init']} - 5`
